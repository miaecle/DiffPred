...
def adjust_contrast(pair_dat, mask, position_code=None, linear_align=False):
    # z-scale phase contrast image to have mean 0 and std 1
    # (Optional) for corner views in 96 well plate, align contrast based on distance to well center
    pc_mat = pair_dat[0]

    if linear_align:
        # Fit a linear model on phase contrast ~ distance to image center
        assert position_code is not None
        dist_mat = generate_dist_mat(mask, position_code)
        dist_segs = np.linspace(
            np.min(dist_mat[np.nonzero(dist_mat)]) - 1e-5, np.max(dist_mat), 7)
...
def generate_weight(mask, position_code, linear_align=True):
    # Define weight matrix based on distance to well center
    if linear_align:
        dist_mat = generate_dist_mat(mask, position_code)
    else:
