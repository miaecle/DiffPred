...
class GradualDefreeze(keras.callbacks.Callback):
    ...
    def on_epoch_begin(self, epoch, logs={}):
        if epoch not in self.key_points:
            return
...
def change_model_level(model, level):
    ...
    model.compile(optimizer='Adam', loss=model.loss, metrics=[])
    return
...
def load_partial_weights(model, model2):
    ...
    layer_names = [layer.name for layer in main_module.layers]
    layer_names2 = [layer.name for layer in main_module2.layers]
    ...
    layer_names2 = [layer.name for layer in model2.model.layers]
    for l_ind, l_name in enumerate(layer_names):
